...
func (s *AWSS3Store) StoreURL(o Object, md5sumHex string) (string, string, error) {
	...
	md5bts, err := hex.DecodeString(md5sumHex)
	if err != nil {
		return "", "", err
	}
	md5s := base64.StdEncoding.EncodeToString(md5bts)
	req.HTTPRequest.Header.Set("Content-MD5", md5s)
	urlStr, err := req.Presign(5 * time.Minute)
	if err != nil {
		return "", "", sdk.WrapError(err, "failed to sign request")
	}
	log.Debug("AWS-S3-Store> StoreURL urlStr:%v key:%v md5sum:%v", urlStr, *key, md5s)
